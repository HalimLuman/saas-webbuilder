...
export const metadata: Metadata = {
  title: "Blog",
  description: "The latest news, tutorials, and updates from BuildStack.",
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-24">
        ...
        <div className="flex items-center justify-center flex-wrap gap-2 mb-12">
          {CATEGORIES.map((cat) => (
            <button
              key={cat}
              className="px-4 py-1.5 rounded-full text-sm font-medium border border-white/10 text-white/50 hover:text-white hover:border-white/30 transition-colors first:bg-white/10 first:text-white first:border-white/20"
            >
              {cat}
            </button>
          ))}
        ...
        {featured && (
          <Link href={`/blog/${featured.slug}`} className="block group mb-12">
            <div className="rounded-3xl overflow-hidden border border-white/10 bg-white/5 hover:bg-white/8 transition-all hover:border-white/20">
              <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-8 lg:p-10 flex flex-col justify-center">
                  ...
                  <h2 className="text-2xl lg:text-3xl font-bold text-white mb-3 group-hover:text-primary-300 transition-colors leading-tight">
                    {featured.title}
                  </h2>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-white/80">{featured.author.name}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {regular.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
              <article className="h-full rounded-2xl border border-white/10 bg-white/5 hover:bg-white/8 hover:border-white/20 transition-all overflow-hidden flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <h3 className="text-base font-bold text-white mb-2 group-hover:text-primary-300 transition-colors leading-tight line-clamp-2">
                    {post.title}
                  </h3>
                  <p className="text-sm text-white/40 leading-relaxed line-clamp-3 flex-1">{post.excerpt}</p>
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5">
                    ...
                    <ArrowRight className="h-3.5 w-3.5 text-white/30 group-hover:text-primary-400 group-hover:translate-x-0.5 transition-all" />
                  </div>
                </div>
              </article>
            </Link>
        ...
        {/* Newsletter CTA */}
        <div className="mt-20 rounded-3xl bg-gradient-to-br from-primary-500/20 via-purple-600/10 to-transparent border border-primary-500/20 p-10 text-center">
          <h2 className="text-2xl font-bold text-white mb-2">Never miss a post</h2>
          <p className="text-white/50 mb-6">Get the latest BuildStack updates and tutorials in your inbox.</p>
          <div className="flex gap-2 max-w-sm mx-auto">
            <input
              type="email"
              placeholder="you@example.com"
              className="flex-1 h-11 px-4 rounded-xl bg-white/10 border border-white/20 text-white placeholder:text-white/30 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
